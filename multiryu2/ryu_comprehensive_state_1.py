...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    Port_Resources = [i for i in range(20000, 40000)]
    PortV2R_Mappings = {}
    Port_Mappings = {}
    forbid_port = 32

    for port in range(20000):
        Port_Mappings[port] = None
    ...
    def TimerEventGen2(self):
        ...
        '''
        while 1:
            self.send_event_to_observers(EventMessage("PORT_SHUFFLE"))
            hub.sleep(30)
    ...
    def update_resources(self,ev):
        ...
        elif message == "PORT_SHUFFLE":
            seed(time())
            pseudo_ranum = randint(0,len(self.Resources)-1)
            print ("Random Number:",pseudo_ranum)
            #random.shuffle(self.Port_Resources)
            for keys in self.Port_Mappings.keys():
                self.Port_Mappings[keys] = self.Port_Resources[pseudo_ranum]
                pseudo_ranum=(pseudo_ranum+1)%len(self.Port_Resources)
            self.PortV2R_Mappings = {v:k for k, v in self.Port_Mappings.items()}
            print ("**********", self.Port_Mappings[80],"***********")
            print ("**********", 80 ,"***********")
    ...
    def isRealPort(self,port):
        '''Returns True Port is real'''
        if port in self.Port_Mappings.keys():
            return True
    
    def isVirtualPort(self,port):
        ''' Returns True if the IP address is virtual'''
        if port in self.Port_Mappings.values():
            return True
    ...
    def handlePacketInEvents(self, ev):
        ...
        if eth.ethertype == ether.ETH_TYPE_IP:
            ...
            if ipv4_pkt.proto == 6:  # 检查UDP协议（17是UDP的协议号）
                ...
                if tcp_pkt:
                    if self.isRealPort(tcp_pkt.src_port): 
                        change_port = self.Port_Mappings[tcp_pkt.src_port]
                        # print("Changing SRC REAL PORT "+ str(tcp_pkt.src_port) +"---> Virtual SRC PORT " + str(change_port))
                        tcp_pkt.src_port = change_port
                    if self.isVirtualPort(tcp_pkt.dst_port):
                        if self.isVirtualIPAddress(dst):
                            if self.isDirectContact(datapath=datapath.id,ipAddr=self.V2R_Mappings[dst]):
                                change_port = self.PortV2R_Mappings[tcp_pkt.dst_port]
                                # print("Changing DST Virtual PORT "+ str(tcp_pkt.dst_port) +"---> Real DST PORT " + str(change_port))
                                tcp_pkt.dst_port = change_port
                    elif self.isRealPort(tcp_pkt.dst_port):
                        if self.isVirtualIPAddress(dst):
                            if not self.isDirectContact(datapath=datapath.id,ipAddr=self.V2R_Mappings[dst]):
                                pktDrop=True
                                print ("Dropping from",dpid)
            ...
            elif ipv4_pkt.proto == 17:
                udp_pkt = pkt.get_protocol(udp.udp)
                if udp_pkt:
                    if self.isRealPort(udp_pkt.src_port): 
                        change_port = self.Port_Mappings[udp_pkt.src_port]
                        # print("Changing SRC REAL PORT "+ str(udp_pkt.src_port) +"---> Virtual SRC PORT " + str(change_port))
                        udp_pkt.src_port = change_port
                    if self.isVirtualPort(udp_pkt.dst_port):
                        if self.isVirtualIPAddress(dst):
                            if self.isDirectContact(datapath=datapath.id,ipAddr=self.V2R_Mappings[dst]):
                                change_port = self.PortV2R_Mappings[udp_pkt.dst_port]
                                # print("Changing DST Virtual PORT "+ str(udp_pkt.dst_port) +"---> Real DST PORT " + str(change_port))
                                udp_pkt.dst_port = change_port
                    elif self.isRealPort(udp_pkt.dst_port):
                        if self.isVirtualIPAddress(dst):
                            if not self.isDirectContact(datapath=datapath.id,ipAddr=self.V2R_Mappings[dst]):
                                pktDrop=True
                                print ("Dropping from",dpid)
